...
final class ControlFrameBufferTests: XCTestCase {
    func testByDefaultAllFramesPassThroughWhenChannelIsWritable() throws {
        ...
        XCTAssertNoThrow(try buffer.processOutboundFrame(.priority, promise: nil, channelWritable: true).assertForward())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.rstStream, promise: nil, channelWritable: true).assertForward())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.settings, promise: nil, channelWritable: true).assertForward())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.pushPromise, promise: nil, channelWritable: true).assertForward())
        ...
        XCTAssertNoThrow(try buffer.processOutboundFrame(.windowUpdate, promise: nil, channelWritable: true).assertForward())
    ...
    func testByDefaultControlFramesAreBufferedWhenNotWritable() throws {
        ...
        XCTAssertNoThrow(try buffer.processOutboundFrame(.priority, promise: nil, channelWritable: false).assertNothing())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.rstStream, promise: nil, channelWritable: false).assertNothing())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.settings, promise: nil, channelWritable: false).assertNothing())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.pushPromise, promise: nil, channelWritable: false).assertNothing())
        ...
        XCTAssertNoThrow(try buffer.processOutboundFrame(.windowUpdate, promise: nil, channelWritable: false).assertNothing())
        ...
        flushedFrames.assertFramesMatch([.headers, .priority, .rstStream, .settings, .pushPromise, .ping, .goAway, .windowUpdate, .altSvc, .origin])
    }
    ...
    func testControlFramesWillQueueEvenIfWritable() throws {
        ...
        XCTAssertNoThrow(try buffer.processOutboundFrame(.priority, promise: nil, channelWritable: true).assertNothing())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.rstStream, promise: nil, channelWritable: true).assertNothing())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.settings, promise: nil, channelWritable: true).assertNothing())
        XCTAssertNoThrow(try buffer.processOutboundFrame(.pushPromise, promise: nil, channelWritable: true).assertNothing())
        ...
        XCTAssertNoThrow(try buffer.processOutboundFrame(.windowUpdate, promise: nil, channelWritable: true).assertNothing())
        ...
        flushedFrames.assertFramesMatch([.headers, .headers, .priority, .rstStream, .settings, .pushPromise, .ping, .goAway, .windowUpdate, .altSvc, .origin])
    }
...
extension HTTP2Frame {
    fileprivate static var data: HTTP2Frame {
        return HTTP2Frame(streamID: 1, payload: .data(.init(data: .byteBuffer(.empty))))
    }

    fileprivate static var headers: HTTP2Frame {
        return HTTP2Frame(streamID: 1, payload: .headers(.init(headers: HPACKHeaders())))
    }

    fileprivate static var priority: HTTP2Frame {
        return HTTP2Frame(streamID: 1, payload: .priority(.init(exclusive: true, dependency: 0, weight: 32)))
    }

    fileprivate static var rstStream: HTTP2Frame {
        return HTTP2Frame(streamID: 1, payload: .rstStream(.protocolError))
    }

    fileprivate static var settings: HTTP2Frame {
        return HTTP2Frame(streamID: .rootStream, payload: .settings(.ack))
    }

    fileprivate static var pushPromise: HTTP2Frame {
        return HTTP2Frame(streamID: 1, payload: .pushPromise(.init(pushedStreamID: 2, headers: HPACKHeaders())))
    }

    fileprivate static var ping: HTTP2Frame {
        return HTTP2Frame(streamID: .rootStream, payload: .ping(HTTP2PingData(), ack: false))
    }

    fileprivate static var goAway: HTTP2Frame {
        return HTTP2Frame(streamID: .rootStream, payload: .goAway(lastStreamID: .maxID, errorCode: .protocolError, opaqueData: nil))
    }

    fileprivate static var windowUpdate: HTTP2Frame {
        return HTTP2Frame(streamID: 1, payload: .windowUpdate(windowSizeIncrement: 1))
    }

    fileprivate static var altSvc: HTTP2Frame {
        return HTTP2Frame(streamID: .rootStream, payload: .alternativeService(origin: nil, field: nil))
    }

    fileprivate static var origin: HTTP2Frame {
        return HTTP2Frame(streamID: .rootStream, payload: .origin([]))
    }
}
